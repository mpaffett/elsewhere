...
test("horizonEndDate lands twelve weeks later", () => {
  // 22 August 2026 plus 84 days is 14 November 2026.
  const start = new Date("2026-08-22T12:00:00Z");
  assert.equal(horizonEndDate(start), "14 November");
});

test("horizonEndDate rolls over the end of a year", () => {
  // 1 December 2026 plus 84 days is 23 February 2027.
  const start = new Date("2026-12-01T12:00:00Z");
  assert.equal(horizonEndDate(start), "23 February");
});
