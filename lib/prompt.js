      ...
      items: {
        ...
        properties: {
          ...
          tonight: {
            type: "string",
            description:
              "One tiny action they could take in the next few hours. Appended after a fixed \"Tonight \u2014 \" lead-in, so it starts LOWERCASE and ends with a period. Imperative mood (\"tune it up and run one chord change\"), never \"you'd...\" or \"you've...\". Must work whatever their starting point: setup, a decision, or a small rep -- never a first-ever-attempt step, and never assume they already own the equipment.",
          },
          byThen: {
            type: "string",
            description:
...
You will be given three amounts of time, each measured over six weeks. For each amount, write TWO things: a tiny action for tonight, and the result they reach by the end of the six weeks.

--- FIELD 1: "tonight" ---

One small action they could genuinely do in the next few hours. This is the whole point of the card: it shows that the goal starts NOW, not someday.

We have already written "Tonight — " before your text. Yours is appended straight after that dash, so it starts LOWERCASE and ends with a period. Write it in the imperative -- an instruction, a nudge, a dare. Not "you'd tune the guitar", not "you've tuned the guitar", just "tune the guitar".

It must be small. Something with no barrier to starting.

Size it to the time that tier frees up TONIGHT -- you're told that figure for each. An evening with 45 minutes free and an evening with two hours free are different evenings, so the three tiers must get three DIFFERENT actions, each one filling the time it actually has. Three identical "tonight" lines side by side tell the reader the tiers don't really differ.
...
FIRST, ask this about the goal: could someone do this at home this evening, with nothing they'd have to buy, book, or travel to?

Some goals genuinely cannot be practised tonight. Sailing needs a boat and water. Flying needs a plane. Pottery needs a wheel and a kiln. Scuba needs a pool and a tank. For any goal like that, tonight's action is PREPARATION -- and preparation is a real, satisfying first step, not a consolation prize.

- Good: "find the three sailing clubs nearest you and note what a taster session costs." (real progress, doable from an armchair)
- Bad: "untie the dock lines and push off from shore." (there is no boat, and no water, tonight)
...
The two must connect. Tonight's action should be the obvious first move toward that six-week result -- the same thread, six weeks apart.
...
export function buildUserMessage(goal, tiers) {
  ...
  // tonight's action (45 minutes and 2 hours are very different evenings),
  // the six-week total sizes the result. Sending only the total meant every
  // tier came back with an identically-sized "tonight".
  const timeLines = tiers
    .map(
      (tier) =>
        `- ${tier.percent}%: ${tier.perDayLabel} free tonight, ${tier.totalHours} hours over the six weeks`,
    )
    .join("\n");
...
Apply THE MOST IMPORTANT RULE to the tagged text above, then write a "tonight" action and a "byThen" result for each of the three amounts.`;
}
